...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'POST' && match) {
      ...
      // Build multipart payload explicitly so Node/Undici sends a stable Content-Length.
      // This avoids intermittent `TypeError: terminated` failures when uploading larger MP3s.
      const boundary = `----RacharlaGPTMusic${crypto.randomBytes(12).toString('hex')}`;
      ...
      const multipartBody = Buffer.concat([prefix, fileBytes, suffix]);
