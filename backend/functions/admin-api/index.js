...
async function uploadStorageFile(bytes, name, mimeType = 'application/octet-stream') {
  ...
  for (let start = 0; start < total; start += CHUNK_SIZE) {
    ...
    // fileId is required on the first request and must be the returned ID
    // on subsequent chunk requests.
    form.append('fileId', fileId || 'unique()');
    form.append('file', new Blob([chunk], { type: mimeType }), name);
    ...
    last = await aw(`/storage/buckets/${encodeURIComponent(BUCKET)}/files`, {
      method: 'POST',
      headers,
      body: form
    });
    ...
    if (!fileId) {
      fileId = String(last?.$id || '');
      if (!fileId) throw Error('Appwrite did not return a file ID for the first upload chunk');
    }
  }
...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'POST' && match) {
      ...
      // Use the native multipart FormData/Blob implementation instead of manually
      // constructing the multipart body. This is more reliable for larger MP3 uploads
      // on Node/Undici and avoids `TypeError: fetch failed/terminated`.
      const form = new FormData();
