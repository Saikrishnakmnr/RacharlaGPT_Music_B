...
const cors = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type,Authorization,X-File-Name,X-Chunk-Start,X-Chunk-End,X-Total-Size,X-Appwrite-ID',
...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'POST' && match) {
      ...
      // IMPORTANT: The browser must send the MP3 to this endpoint in small chunks.
      // Sending a 12+ MB body to an Appwrite Function can fail at the Appwrite
      // gateway before the function code is reached ("backend read error").
      const bytes = req.bodyBinary;
      const startRaw = req.headers['x-chunk-start'];
      const endRaw = req.headers['x-chunk-end'];
      const totalRaw = req.headers['x-total-size'];
      const fileId = String(req.headers['x-appwrite-id'] || '').trim();
      ...
      if (!bytes?.length) return bad(res, 'MP3 chunk is empty');
      if (!Number.isSafeInteger(start) || !Number.isSafeInteger(end) || !Number.isSafeInteger(total)) {
        return bad(res, 'Invalid MP3 chunk range');
      }
      if (start < 0 || total <= 0 || end <= start || end > total || bytes.length !== end - start) {
        return bad(res, 'MP3 chunk range does not match the uploaded bytes');
      }
      if (bytes.length > MAX_CHUNK) return bad(res, 'MP3 chunk is too large');
      if (total > 30 * 1024 * 1024) return bad(res, 'MP3 is too large');

      const name = (req.headers['x-file-name'] || `${orderNumber}-final.mp3`)
        .replace(/[^a-zA-Z0-9._-]/g, '_');
      if (!/\.mp3$/i.test(name)) return bad(res, 'Final file must be an MP3');

      // Appwrite Storage supports resumable uploads with Content-Range and
      // X-Appwrite-ID. The first chunk creates the file; later chunks append.
      ...
      const uploaded = await aw(`/storage/buckets/${encodeURIComponent(BUCKET)}/files`, {
        method: 'POST',
        headers,
        body: form
      });

      const actualFileId = String(uploaded?.$id || fileId || '');
      if (!actualFileId) throw Error('Appwrite did not return a file ID');

      const complete = end === total;
      if (complete) {
        const existing = (await rows(FILES)).find(x =>
          String(x?.order_number) === orderNumber &&
          String(x?.file_id) === actualFileId
        );
        ...
        await create(EVENTS, {
          order_number: orderNumber,
          event_type: 'SONG_READY',
          message: 'Final MP3 uploaded and order marked READY.',
          created_at: new Date().toISOString(),
          created_by: 'admin'
        });
      }
