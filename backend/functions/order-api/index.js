...
const cors = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type,X-Access-Token,X-Customer-Email,X-File-Type,X-File-Name',
...
async function serviceSettings() {
  const row = (await rows(OFFERS)).find(r => String(r?.name || '').toUpperCase() === 'SERVICE_STATUS');
  const enabled = row ? row.active !== false && String(row.code || 'ENABLED').toUpperCase() === 'ENABLED' : true;
  return { enabled, message: 'We are currently unable to accept new song orders. Please come back later.' };
}
...
async function orderByNo(no, access, customerEmail = '') {
  const r = await findRow(ORDERS, 'order_number', no);
  if (!r) return null;
  const accessOk = !!access && same(hash(access), r.access_token_hash || '');
  const email = String(customerEmail || '').trim().toLowerCase();
  const orderEmail = String(r.email || '').trim().toLowerCase();
  const emailOk = !!email && email === orderEmail;
  if (!accessOk && !emailOk) return null;
  return r;
}
...
async function sendOrderConfirmationEmail(order, paymentId = '') {
  if (!RESEND_API_KEY || !EMAIL_FROM) throw Error('Automatic email service is not configured in order-api');
  const orderNumber = String(order.order_number || '');
  const customerName = String(order.customer_name || 'Customer');
  const email = String(order.email || '').trim().toLowerCase();
  const occasion = String(order.occasion || 'Personalized Song');
  const amount = Number(order.price || 0);
  const trackUrl = `${SITE}/track-order.html`;
  const subject = `RacharlaGPT Music — Order ${orderNumber} confirmed`;
  ...
  const html = `<!doctype html><html><body style="margin:0;background:#f6f4f8;font-family:Arial,sans-serif;color:#211b2b"><div style="max-width:620px;margin:30px auto;background:#fff;border:1px solid #e7e1ec;border-radius:16px;padding:28px"><h2 style="margin-top:0">RacharlaGPT Music</h2><p>Hello ${escapeHtml(customerName)},</p><p>Your personalized song order has been confirmed.</p><p><strong>Order Number:</strong> ${escapeHtml(orderNumber)}<br><strong>Occasion:</strong> ${escapeHtml(occasion)}<br><strong>Amount Paid:</strong> ₹${escapeHtml(amount)}<br><strong>Payment:</strong> Confirmed</p><p>Your song is now in our production process.</p><p><a href="${trackUrl}" style="display:inline-block;padding:12px 18px;border-radius:10px;background:#6d54ff;color:#fff;text-decoration:none;font-weight:700">Track My Song</a></p><p>Enter your RM Order Number and the email address used for your order.</p><p>Please keep your RM number for future reference.</p><p>Thank you for choosing RacharlaGPT Music.<br><strong>Your Idea. Your Song. Your World.</strong></p></div></body></html>`;
  const r = await fetch('https://api.resend.com/emails', {
    method: 'POST',
    headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from: EMAIL_FROM, to: [email], subject, text, html, ...(paymentId ? { headers: { 'X-Entity-Ref-ID': orderNumber } } : {}) })
  });
  const raw = await r.text();
  let data = {};
  try { data = raw ? JSON.parse(raw) : {}; } catch { data = { raw }; }
  if (!r.ok) throw Error(`Email provider ${r.status}: ${raw}`);
  return data;
}
async function markPaymentPaid(order, paymentId, razorpayOrderId, source = 'razorpay') {
  ...
  if (!wasPaid) {
    ...
    try {
      await sendOrderConfirmationEmail(order, paymentId);
      await event(order.order_number, 'CUSTOMER_EMAIL_SENT', 'Automatic payment confirmation email sent to the customer.', 'system', {
        email: order.email
      });
    } catch (mailError) {
      console.error('Automatic confirmation email failed:', mailError);
      await event(order.order_number, 'CUSTOMER_EMAIL_FAILED', 'Automatic payment confirmation email could not be sent.', 'system', {
        email: order.email,
        error: String(mailError?.message || mailError).slice(0, 500)
      });
    }
  }
  return now;
...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'GET' && path === '/pricing') {
      const service = await serviceSettings();
      return json(res, { price: price(), normal_price: NORMAL, campaign_active: active(), campaign_name: active() ? CAMPAIGN_NAME : null, currency: 'INR', service_enabled: service.enabled, service_message: service.message });
    }
    ...
    if (req.method === 'POST' && path === '/orders') {
      const service = await serviceSettings();
      if (!service.enabled) return bad(res, service.message, 503);
      const b = req.bodyJson || {};
    ...
    if (req.method === 'GET' && m) {
      ...
      const o = await orderByNo(no, t, customerEmail);
    ...
    if (req.method === 'POST' && m) {
      const no = decodeURIComponent(m[1]);
      const customerEmail = req.headers['x-customer-email'] || '';
      const o = await orderByNo(no, req.headers['x-access-token'], customerEmail);
      if (!o) return bad(res, 'Order not found', 404);
