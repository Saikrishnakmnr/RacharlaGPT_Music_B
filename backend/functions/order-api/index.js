...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'POST' && path === '/recover') {
      ...
      // Recovery verifies the payment directly with Razorpay. If it is captured,
      // synchronize the order payment state before issuing the fresh private link.
    ...
    if (req.method === 'GET' && m) {
      ...
      const ms = (await rows(MESSAGES)).filter(x => String(x?.order_number) === no).sort((a,b) => String(a.created_at).localeCompare(String(b.created_at)));
      const es = (await rows(EVENTS)).filter(x => String(x?.order_number) === no).sort((a,b) => String(a.created_at).localeCompare(String(b.created_at)));
      const fs = (await rows(FILES)).filter(x => String(x?.order_number) === no).map(x => ({ file_type: x.file_type, file_name: x.file_name, mime_type: x.mime_type || null, created_at: x.created_at }));
    ...
    if (req.method === 'POST' && m) {
      ...
      await create(MESSAGES, { order_number: no, sender_type: 'customer', message: body, created_at: new Date().toISOString() });
