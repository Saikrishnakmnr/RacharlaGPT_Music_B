...
async function customerOrderByEmail(no, email) {
  const r = await findRow(ORDERS, 'order_number', no);
  const normalized = String(email || '').trim().toLowerCase();
  if (!r || !normalized || String(r.email || '').trim().toLowerCase() !== normalized) return null;
  return r;
}
async function authorizeOrder(req, no) {
  const access = req.headers['x-access-token'] || new URLSearchParams(req.queryString || '').get('token');
  if (access) return orderByNo(no, access);
  return customerOrderByEmail(no, req.headers['x-customer-email'] || '');
}
...
async function sendOrderConfirmationEmail(order) {
  if (!RESEND_API_KEY || !EMAIL_FROM || !order?.email) return { sent: false, skipped: true };
  const subject = `RacharlaGPT Music — Order ${order.order_number} confirmed`;
  const safeName = String(order.customer_name || 'Customer');
  const text = [
    `Hello ${safeName},`,
    '',
    'Thank you for ordering your personalized song from RacharlaGPT Music.',
    '',
    `Order Number: ${order.order_number}`,
    `Occasion: ${order.occasion}`,
    `Amount Paid: ₹${Number(order.price || 0).toLocaleString('en-IN')}`,
    'Payment: Confirmed',
    '',
    'Your song is now in our production process.',
    '',
    'To check your order anytime, open:',
...
async function markPaymentPaid(order, paymentId, razorpayOrderId, source = 'razorpay') {
  ...
  if (!wasPaid) {
    ...
    try {
      const mail = await sendOrderConfirmationEmail({ ...order, payment_status: 'PAID', paid_at: now });
      if (mail.sent) await event(order.order_number, 'CUSTOMER_EMAIL_SENT', 'Order confirmation email sent to the customer.', 'system');
      else await event(order.order_number, 'CUSTOMER_EMAIL_SKIPPED', 'Order confirmation email is not configured yet.', 'system');
    } catch (mailError) {
      await event(order.order_number, 'CUSTOMER_EMAIL_FAILED', 'Order confirmation email could not be sent.', 'system', { error: String(mailError?.message || mailError) });
    }
  }
  return now;
}
...
export default async ({ req, res, error }) => {
  try {
    ...
    if (req.method === 'POST' && path === '/recover') {
      ...
      return json(res, { ok: true, order_number: o.order_number, payment_status: 'PAID' });
    }
    ...
    if (req.method === 'POST' && path === '/orders') {
      ...
      return json(res, {
        order_number: n, access_token: t, customer_name: name, email,
        amount, currency: 'INR', campaign, campaign_active: active(),
    ...
    if (req.method === 'GET' && m) {
      ...
      const o = await authorizeOrder(req, no);
    ...
    if (req.method === 'POST' && m) {
      ...
      return json(res, { ok: true, payment_status: 'PAID', paid_at: o.paid_at || now, order_number: no });
    }
