...
const cors = {
  ...
  'Access-Control-Allow-Headers':
    'Content-Type,X-Access-Token,X-File-Type,X-File-Name',
...
async function aw(path, opt = {}) {
  if (!ENDPOINT || !PROJECT || !KEY) {
    throw Error('Appwrite environment is not configured');
  }
  ...
  if (!r.ok) {
    throw Error(`Appwrite ${r.status}: ${d.message || t}`);
  }

  return d;
}

// Read rows without a TablesDB filter query. This avoids the current
// REST query/schema error while keeping the existing table structure unchanged.
async function rows(table) {
  return (
    await aw(
      `/tablesdb/${DB}/tables/${table}/rows?limit=100`
    )
  ).rows || [];
}

async function rowsWhere(table, column, value) {
  const wanted = String(value);
  return (await rows(table)).filter(
    (row) => String(row?.[column] ?? '') === wanted
  );
}

async function create(table, data) {
  return aw(`/tablesdb/${DB}/tables/${table}/rows`, {
    method: 'POST',
...
async function update(table, id, data) {
  return aw(`/tablesdb/${DB}/tables/${table}/rows/${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
async function orderByNo(no, access) {
  const r = (await rowsWhere(ORDERS, 'order_number', no))[0];

  if (!r || !same(hash(access || ''), r.access_token_hash || '')) {
    return null;
  }
...
async function rzpOrder(receipt, amount) {
  ...
  const auth =
    'Basic ' +
    Buffer.from(`${RZP_ID}:${RZP_SECRET}`).toString('base64');
  ...
  if (!r.ok) {
    throw Error(
      d.error?.description || 'Payment order creation failed'
    );
  }
...
async function fileToken(fileId) {
  ...
  const d = await aw(
    `/tokens/buckets/${BUCKET}/files/${encodeURIComponent(fileId)}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
  ...
  return (
    `${ENDPOINT.replace(/\/$/, '')}/v1/storage/buckets/` +
    `${encodeURIComponent(BUCKET)}/files/${encodeURIComponent(fileId)}` +
    `/download?project=${encodeURIComponent(PROJECT)}` +
    `&token=${encodeURIComponent(d.secret)}`
  );
}

/*
 * Appwrite table: order_events
 * Uses:
 *   order_number
 *   event_type
 *   message
...
async function event(order, eventType, message = null, createdBy = null, metadata = null) {
  try {
    ...
    // Event logging should not break the main customer request.
  }
}

/*
 * Appwrite table: messages
 * Uses:
 *   order_number
 *   sender_type
 *   message
  ...
  try {
    ...
    if (req.method === 'GET' && path === '/pricing') {
      return json(res, {
        ...
        campaign_name: active() ? CAMPAIGN_NAME : null,
        currency: 'INR'
      });
    }
    ...
    if (req.method === 'POST' && path === '/orders') {
      ...
      if (!name || !email || !occasion) {
        return bad(res, 'Name, email and occasion are required');
      }
      ...
      if (!['type', 'upload', 'voice'].includes(input)) {
        return bad(res, 'Invalid input method');
      }
      ...
      await create(ORDERS, {
        ...
        access_token_hash: hash(t),
        created_at: new Date().toISOString(),
        revision_count: 0,
        download_count: 0
      });

      await event(n, 'ORDER_CREATED');
      ...
      return json(res, {
        ...
        track_url:
          `${SITE}/track.html?order=${encodeURIComponent(n)}` +
    ...
    if (req.method === 'GET' && m) {
      ...
      const ms = await rowsWhere('messages', 'order_number', no).catch(() => []);
    ...
    if (req.method === 'POST' && m) {
      ...
      if (!['voice', 'paper'].includes(type)) {
        return bad(res, 'Invalid file type');
      }

      if (!bytes?.length || bytes.length > 12 * 1024 * 1024) {
        return bad(res, 'File is empty or too large');
      }
      ...
      /*
       * Appwrite table: order_files
       * Uses file_type, not type.
       */
      await create(FILES, {
        order_number: no,
        file_type: type,
    ...
    if (req.method === 'POST' && m) {
      ...
      if (!body || body.length > 2000) {
        return bad(res, 'Message is empty or too long');
      }

      /*
       * Appwrite table: messages
       * Uses sender_type and message, not sender_role/body.
       */
    ...
    if (req.method === 'GET' && m) {
      ...
      if (
        ...
        return bad(res, 'Song is not ready', 404);
      }
       ...
       */
      const fs = (
        await rowsWhere(FILES, 'order_number', no)
      )
        .filter(
          (x) => x.file_type === 'mp3'
        )
      ...
      if (!fs[0]) {
        return bad(res, 'Song file not found', 404);
      }

      const url = await fileToken(
        fs[0].file_id
      );
      ...
      await event(
        no,
        'SONG_DOWNLOAD_LINK_OPENED'
      );

      return json(res, {
        ok: true,
        file_name: fs[0].file_name,
        url
      });
    }
